...
func UpdateValidateBook(id uint, book *models.Book) error {
	if book.Title == "" {
		return errors.New("title is required")
	}
	...
	// unique validate
	var existingTitle models.Book
	if err := config.DB.Where("title = ? AND id != ?", book.Title, id).First(&existingTitle).Error; err == nil {
		return errors.New("title already exists")
	}
